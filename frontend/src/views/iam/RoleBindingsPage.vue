...
const roles = useRoleList(auth.can('tenancy.tenant:create') ? 'platform' : 'tenant')
...
const form = reactive({ user_id: '', role_id: '', scope_type: 'tenant', scope_id: '' })
  ...
  try {
    await m03CreateRoleBinding({
      body: {
        ...
        scope: { type: form.scope_type as 'platform' | 'tenant' | 'project', id: scopeId },
      },
      throwOnError: true,
    })
...
async function removeBinding(b: RoleBindingOut) {
  try {
    await m03DeleteRoleBinding({ path: { binding_id: b.binding_id }, throwOnError: true })
...
function openInspector(userId?: string) {
  ...
  if (!inspector.scope_id && auth.activeTenantId) inspector.scope_id = auth.activeTenantId
  shell.openPanel()
}
  ...
  try {
    const { data } = await m03EffectivePermissions({
      ...
      query: {
        ...
        scope_id: inspector.scope_type === 'platform' ? null : inspector.scope_id || null,
      },
      throwOnError: true,
    })
  ...
  <div>
    <div class="page-header-row">
      ...
      <div class="header-actions">
        <button class="btn btn-secondary" type="button" @click="openInspector()">
    ...
    <div v-if="bindings.loading.value" class="skeleton" style="height: 280px" />
    <div v-else-if="bindings.items.value.length === 0" class="empty-state card">
      No role bindings visible at this scope.
    </div>
    <table v-else class="data-table">
      <thead>
        <tr><th>User</th><th>Role</th><th>Scope</th><th>Origin</th><th></th></tr>
      </thead>
      <tbody>
        <tr v-for="b in bindings.items.value" :key="b.binding_id">
          <td class="text-sm-medium">{{ b.user_display_name ?? b.user_id }}</td>
          <td>{{ b.role_name ?? b.role_id }}</td>
          <td><span class="tag-chip">{{ b.scope.type }}{{ b.scope.id ? `:${b.scope.id}` : '' }}</span></td>
          <td class="text-caption">{{ b.origin }}</td>
          <td>
            <span class="row-actions">
              <button class="btn btn-ghost btn-sm" type="button" @click="openInspector(b.user_id)">
                Inspect
              </button>
              <button
                v-if="auth.can('iam.role_binding:delete') && b.origin === 'direct'"
    ...
    <Teleport to="#shell-panel-body" defer>
      <div v-if="shell.panelOpen">
        <SettingsSection title="Effective permissions">
          <DropdownRow
            v-model="inspector.user_id"
            label="User"
            :options="users.items.value.map((u) => ({ value: u.user_id, label: u.display_name }))"
          ...
          />
          <div v-if="inspector.scope_type !== 'platform'" class="field">
            <label class="field-label">Scope id</label>
            <input v-model="inspector.scope_id" class="text-input" placeholder="tnt-… / prj-…" />
          </div>
          <button class="btn btn-primary btn-sm" type="button" :disabled="inspecting" @click="runInspection">
        ...
        <SettingsSection v-if="inspectorResult" title="Result">
          <p class="text-caption">
            user {{ inspectorResult.user_status }} · scope {{ inspectorResult.scope_status }}
            <template v-if="inspectorResult.denied_reasons.length">
              · denied: {{ inspectorResult.denied_reasons.join(', ') }}
            </template>
          </p>
          <div v-if="inspectorResult.allowed.length === 0" class="text-caption">
            No permissions at this scope.
          </div>
          <div v-for="entry in inspectorResult.allowed" :key="entry.key" class="perm-row">
            <code class="perm-key">{{ entry.key }}</code>
            <span class="text-caption" :title="entry.via.map((v) => `via ${v.role} @ ${v.scope}`).join('\n')">
              via {{ entry.via[0]?.role }} @ {{ entry.via[0]?.scope }}
            </span>
          </div>
        </SettingsSection>
      </div>
    ...
    <ModalDialog title="New role binding" :open="createOpen" @close="createOpen = false">
      <p v-if="createError" class="banner-error">{{ createError }}</p>
      <DropdownRow
        v-model="form.user_id"
        label="User"
        :options="users.items.value.map((u) => ({ value: u.user_id, label: `${u.display_name} (${u.email})` }))"
        placeholder="Select user…"
      />
      <DropdownRow
        ...
        :options="[
          ...
          ...roles.items.value
            ...
            .map((r) => ({ value: r.role_id, label: `${r.name} (custom)` })),
        ]"
      ...
      <div v-if="form.scope_type !== 'platform'" class="field">
        ...
        <input
          ...
          class="text-input"
          :placeholder="form.scope_type === 'tenant' ? (auth.activeTenantId ?? 'tnt-…') : 'prj-…'"
        />
      ...
      <template #footer>
        ...
        <button class="btn btn-primary" type="button" @click="createBinding">Create binding</button>
      </template>
    </ModalDialog>
  </div>
</template>
